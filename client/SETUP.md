...
The activity signal is deliberately keyboard-first, not mouse — a mouse can get bumped by someone walking past a desk, a key press is a much stronger signal. It's installed via a low-level keyboard hook (the `keyboard` package), which only ever looks at *timing* of key presses, never which keys were pressed. Some corporate endpoint security software may flag or silently filter low-level keyboard hooks regardless of intent, the same way `schtasks` got blocked — so a second, independent signal (Windows' own combined mouse+keyboard idle-time API) always runs alongside it as a fallback. If a return-from-idle is detected via mouse only (no key press), it isn't trusted immediately — it waits up to `MOUSE_CONFIRM_WINDOW_SECONDS` (default 120s) for a real key press to join in before actually posting a checkin. If no key press shows up in that window, the mouse blip is discarded as a false alarm and the 2-hour idle clock is *not* reset — it keeps waiting for a genuine return rather than requiring another full 2 hours of idle time. Check `checkin_client.log` for lines confirming whether the keyboard hook installed, and (if it ever happens) whether a return was confirmed via keyboard, ignored as a mouse-only false alarm, or fired immediately via keyboard.
